...
template <typename T>
void DXFExport::exportClustersToDXF(const std::string& file_name, const TVector <Facility>& F, const TVector <Point3D>& points,  const TVector <RegressionPlane>& ANN)
{
	...
	try
	{
		...
		for (Facility f : F)
		{
			...
			for (auto c_id : f.Cl_id)
			{
				...
				createLine(file, level_normals, points[c_id2].x, points[c_id2].y, points[c_id2].z, points[c_id2].x + 0.1 * ANN[c_id2].a, points[c_id2].y + 0.1 * ANN[c_id2].b, points[c_id2].z + 0.1 * ANN[c_id2].c, color_normals);
			}

			//Create normal at facility
			createLine(file, level_normals, points[u_id2].x, points[u_id2].y, points[u_id2].z, points[u_id2].x + 0.1 * ANN[u_id2].a, points[u_id2].y + 0.1 * ANN[u_id2].b, points[u_id2].z + 0.1 * ANN[u_id2].c, color_normals);
		}
