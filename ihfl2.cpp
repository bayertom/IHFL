...
double IHFL2::dist(const double x1, const double y1, const double z1, const double x2, const double y2, const double z2)
{
...
double IHFL2::pointPlaneDist(const double qx, const double qy, const double qz, const double a, const double b, const double c, const double d)
{
...
double IHFL2::abn(const RegressionPlane& pa, const RegressionPlane& pb)
{
...
double IHFL2::ablp(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
...
double IHFL2::dfp(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
...
double IHFL2::nL2(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
	//L2 norm
...
double IHFL2::nABN(const Point3D& a, const Point3D& b, const RegressionPlane & pa, const RegressionPlane & pb)
{
...
double IHFL2::nDIS(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
...
double IHFL2::nABLP(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
...
double IHFL2::nDFP(const Point3D& a, const Point3D& b, const RegressionPlane& pa, const RegressionPlane& pb)
{
...
void IHFL2::generateClusters(const double w, const double h, const double rad, const int nc, const int n, TVector <Point3D>& U)
{
...
void IHFL2::sideConePoint(const double a, const double b, double& h, double& r, double& t)
{
...
void IHFL2::baseConePoint(const double a, const double b, double& h, double& r, double& t)
{
...
void IHFL2::sideCylinderPoint(const double a, const double b, double& h, double& r, double& t)
{
...
void IHFL2::baseCylinderPoint(const double a, const double b, double& h, double& r, double& t)
{
	//Points on the base of cone
	std::random_device rd;
	std::mt19937 gen(rd());
...
void IHFL2::generateCone(const double a, const double b, const int n, TVector <Point3D>& U)
{
...
void IHFL2::generateCylinder(const double a, const double b, const int n, TVector <Point3D>& U)
{
	//Generate cone
	double t = 0;
...
void IHFL2::generateCube(const double a, const int n, TVector <Point3D>& U)
{
...
void IHFL2::recomputeFacilityCosts(const double fc, double rat, const TVector <RegressionPlane>& RP, const pfnorm& fnorm, TVector <Point3D> &U)
{
	...
	{
		//ABN
		if (fnorm == &IHFL2::nDIS || fnorm == &IHFL2::nABN || fnorm == &IHFL2::nABLP)
		{
		...
		//DFP + L2
		else if (fnorm == &IHFL2::nDFP || fnorm == &IHFL2::nL2)
		{
...
void IHFL2::getAveragePointNormal(TVector <Point3D>& U, const TVector2D <size_t>& knn_id, TVector <RegressionPlane>& RP)
{
...
void IHFL2::clusterizeIHFL(TVector <Point3D>& U, const double fc, TVector<Facility>& F, TVector <RegressionPlane>& RP)
{
	...
	GridIndexing gi(0.2, 0.2, 0.2);
	gi.initializeIndex(U);
	TVector2D <std::shared_ptr <Facility> > FF(gi.nx * gi.ny * gi.nz);
	TVector2D <Facility> FF2(gi.nx * gi.ny * gi.nz);

	//First point becomes the facility
	std::shared_ptr <Facility> f0 = std::make_shared<Facility>(U[0].id + 1, U[0].fc);
	Facility ff0(U[0].id + 1, U[0].fc);

	//Compute grid index
	int idx_p = gi.get1DIndex(U[0]);

	//Add facility to the grid index
	FF[idx_p].push_back(f0);
	FF2[idx_p].push_back(ff0);
	F.push_back(ff0);
	...
	for (int i = 1; i < n; i++)
	{
		...
		updateClusters3(i, U, RP, gi, FF2);
		//updateClusters2(i, U,RP, gi, FF);
		//updateClusters(i, U, F, RP);
	}

	//Convert indexed grid to the list
	for (auto ff2: FF2)
		F.insert(F.end(), std::make_move_iterator(ff2.begin()), std::make_move_iterator(ff2.end()));
	

	
	//TVector <Facility> F0 = FF2 | std::views::transform([](const auto& i) { return i[0]; });
	/*
	for (auto FFB : FF)
		for (auto f : FFB)
			F.push_back(*f);
...
void IHFL2::updateClusters(const int i, const TVector <Point3D>& points, TVector <Facility>& F, const TVector <RegressionPlane>& RP)
{
	...
	else
	{
		...
		{
			//Reset sign of the reallocated cluster ID and shift
	...
	if (c_min == c_reallocate_clusters)
		F.erase(std::remove_if(F.begin(), F.end(), IsFacilitySetForDeletion()), F.end());
}


void IHFL2::updateClusters2(const int i, const TVector <Point3D>& points, const TVector <RegressionPlane>& RP, GridIndexing& gi, TVector2D <std::shared_ptr <Facility> >& F)
{
	//Incremental method, update of the clusterization takes one of four strategies:
	//   1) Create new facility at p: S1
	//   2) Connect p to the (pseudo) nearest facility: S2
	...
	//Initialize index
	std::pair<int, int> idx_nearest = { -1, -1 };
	...
	//Get bin index of the added point
	int idx_p = gi.get1DIndex(points[i]);

	//Get indices of all adjacent bins
	TVector <int> idxs_fac = gi.getAdjacentIndices(points[i]);

	//Process all neighboring bins
	TVector <std::pair <int, int> > reallocate_clusters;
	for (int idx_fac : idxs_fac)
	{
		for (int j = 0; j < F[idx_fac].size(); j++)
		{
			//Get facility
			std::shared_ptr<Facility> &fac = F[idx_fac][j];
			...
			if (dist_pf < 3.0 * lambda)
			{
				...
				{
					c_nearest = dpf;
					idx_nearest.first = idx_fac;
					idx_nearest.second = j;
				}
				...
				if ((dc_all < dc_closer))
				{
					...
					//Change facility ID to +
					fac->p_idx = abs(fac->p_idx);

					//Set ID of the reallocated cluster
					reallocate_clusters.push_back(std::pair<int, int>(idx_fac, j));
				}
	...
	if (c_min == c_new)
	{
		std::shared_ptr<Facility> fac_new = std::make_shared<Facility>(i + 1, points[i].fc);
		F[idx_p].push_back(fac_new);
	}

	//S2: Connect p to the nearest facility
	else if (c_min == c_nearest)
	{
		F[idx_nearest.first][idx_nearest.second]->U_idxs.push_back(i + 1);
	}
	...
	else
	{
		...
		//Process all clusters affected by the reallocation one by one
		for (const auto &k : reallocate_clusters)
		{
			//Partial realloation: S4
			if (F[k.first][k.second]->p_idx < 0)
			{
				//Reconnect all clients
				TVector <int> U_old_idxs;
				for (const int& u_idx : F[k.first][k.second]->U_idxs)
				{
					//Reconnect client to the new facility
					if (u_idx > 0)
						fac_new->U_idxs.push_back(abs(u_idx));
				...
				//Connect remaining points to the old facility (faster then delete)
				F[k.first][k.second]->U_idxs = std::move(U_old_idxs);
			}
			...
			{
				//Connect all clients of the old facility to the new facility fp at p
				fac_new->U_idxs.insert(fac_new->U_idxs.end(), std::make_move_iterator(F[k.first][k.second]->U_idxs.begin()), std::make_move_iterator(F[k.first][k.second]->U_idxs.end()));

				//Connect old facility to the new facility fp at p
				fac_new->U_idxs.push_back(abs(F[k.first][k.second]->p_idx));

				//Mark old facility for the deletetion
				F[k.first][k.second]->del = true;
			}
		}
	...
	//Delete marked facilities of the old cluster (now it is reallocated)
	for (int idx_fac : idxs_fac)
		F[idx_fac].erase(std::remove_if(F[idx_fac].begin(), F[idx_fac].end(), IsFacilitySetForDeletion()), F[idx_fac].end());

}

void IHFL2::updateClusters3(const int i, const TVector <Point3D>& points, const TVector <RegressionPlane>& RP, GridIndexing& gi, TVector2D <Facility >& F)
{
	//Incremental method, update of the clusterization takes one of four strategies:
	//   1) Create new facility at p: S1
	//   2) Connect p to the (pseudo) nearest facility: S2
...
double IHFL2::computeCost(const TVector <Facility>& F, const TVector <Point3D> &points, const TVector <RegressionPlane>& AN)
{
...
void IHFL2::clusterStatistics(const TVector <Point3D>& points, const TVector <Facility>& F, const TVector <RegressionPlane>& RP, TVector <int>& NC, TVector <double> &RAD, TVector <double>& ABN, TVector <double>& DFP, TVector <double>& ASP, TVector <int>& DIM, TVector <int>& OVER, TVector <double>& SLO)
{
